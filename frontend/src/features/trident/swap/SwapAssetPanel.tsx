...
> = ({ walletToggle, currency, onSelect, spendFromWallet, id, currencies }) => {
  ...
  const trigger = currency ? (
    <div
      id={id}
      className="text-indigo-900 bg-white inline-flex items-center pl-4 pr-2 py-2 rounded-md hover:bg-indigo-800 hover:text-white focus:outline-none"
    >
      <CurrencyLogo currency={currency} className="!rounded-full overflow-hidden" size={20} />
      <Typography variant="sm" className="ml-2 font-bold uppercase" weight={700}>
        {!spendFromWallet ? currency.wrapped.symbol : currency.symbol}
      </Typography>
      <ChevronDownIcon width={18} />
    </div>
  ) : (
    <Button color="blue" variant="filled" size="sm" id={id} className="rounded-md !px-2 !py-0 !h-[32px] !pl-3">
